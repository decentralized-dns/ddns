...
async def register_status(
    request: Request, register_name: str = Form(...), register_duration: str = Form(...)
):
...
async def extend_status(
    request: Request, extend_name: str = Form(...), extend_duration: str = Form(...)
):
    extend_status = extend_name(extend_name, extend_duration)
    return templates.TemplateResponse(
        "manage.html", {"request": request, "extend_status": extend_status}
    )
...
async def renew_status(
    request: Request, renew_name: str = Form(...), renew_duration: str = Form(...)
):
    renew_status = renew_name(renew_name, renew_duration)
    return templates.TemplateResponse(
        "manage.html", {"request": request, "renew_status": renew_status}
    )
...
async def connect_pera(request: Request):
    ...
    response = requests.post(
        "https://api.algosigner.io/v2/wallets",
